...
function Overview({ report, gameDir, stage, release, releaseState, allReady, desktop, onChoose, onInstall, onLauncher }: { report:DetectionReport; gameDir:string; stage:Stage; release:ModRelease|null; releaseState:ReleaseState; allReady:boolean; desktop:boolean; onChoose:()=>void; onInstall:()=>void; onLauncher:()=>void }) {
  ...
  return <div className="flex flex-col gap-5">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><StatusCard label="MINECRAFT" checked={desktop} ok={report.minecraftFound} detail={report.minecraftVersion ?? 'Não detectada'}/><StatusCard label="JOGO ABERTO" checked={desktop} ok={report.minecraftRunning} detail={report.minecraftRunning ? 'Processo Java/Minecraft ativo' : 'Processo não detectado'}/><StatusCard label="FABRIC" checked={desktop} ok={report.fabricInstalled} detail={report.fabricLoaderVersion ?? 'Instale manualmente'}/><StatusCard label="FABRIC API" checked={desktop} ok={report.fabricApiInstalled} detail={report.fabricApiVersion ?? 'Instale manualmente'}/><StatusCard label="AUTOMINER" checked={desktop} ok={report.autominerInstalled} detail={report.autominerVersion ?? 'Não detectada'}/></div>
    ...
    <Card><CardHeader><CardTitle className="text-base">Fluxo seguro</CardTitle><CardDescription>O Fabric Loader e o Fabric API são instalados pelo usuário. O Manager altera somente o AutoMiner.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-3">{[['1','Detectar','Valida arquivos, Minecraft 1.21.1 e Fabric.'],['2','Instalar','Copia somente o JAR do AutoMiner para o destino correto.'],['3','Monitorar','Detecta o processo Java/Minecraft sem telemetria.']].map(([n,t,c])=><div key={n} className="flex gap-3 rounded-lg border p-4"><span className="font-mono text-primary">{n}</span><div><p className="text-sm font-medium">{t}</p><p className="text-xs leading-5 text-muted-foreground">{c}</p></div></div>)}</CardContent></Card>
  </div>
}

function MinecraftView({ report, gameDir, onChoose, onDetect }:{report:DetectionReport;gameDir:string;onChoose:()=>void;onDetect:()=>void}) { return <div className="flex flex-col gap-5"><Heading title="Minecraft" copy="Instalação identificada pela estrutura, arquivos e versão — nunca pelo nome da pasta."/><Card><CardHeader><CardTitle className="text-base">Instalação ou instância selecionada</CardTitle><CardDescription className="font-mono text-xs">{gameDir || 'Não detectada'}</CardDescription>{report.modsDir && <CardDescription className="font-mono text-xs">Destino do mod: {report.modsDir}</CardDescription>}</CardHeader><CardContent className="flex flex-wrap gap-2"><Button onClick={onChoose}><Folder data-icon="inline-start"/>Selecionar pasta</Button><Button variant="outline" onClick={onDetect}><RefreshCw data-icon="inline-start"/>Detectar novamente</Button></CardContent></Card><div className="grid gap-3 md:grid-cols-2">{[['Minecraft 1.21.1',report.version1211],['Minecraft aberto',report.minecraftRunning],['Fabric Loader instalado pelo usuário',report.fabricInstalled],['Fabric API instalado pelo usuário',report.fabricApiInstalled]].map(([label,ok])=><StatusRow key={String(label)} label={String(label)} ok={Boolean(ok)}/>)}</div></div> }
