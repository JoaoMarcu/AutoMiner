...
public final class AutoMinerController implements AutoCloseable {
  ...
  private volatile AutoMinerConfig config; private volatile boolean hudVisible; private LocalApiServer api; private BlockPos target; private float baseYaw; private long startedAt,lastAction;
  ...
  private void updateTelemetry(MinecraftClient c){telemetry.activeSeconds=startedAt==0?0:(System.currentTimeMillis()-startedAt)/1000;telemetry.player=new Telemetry.Vec(c.player.getX(),c.player.getY(),c.player.getZ());telemetry.camera=new Telemetry.Cam(c.player.getYaw(),c.player.getPitch());telemetry.speed=c.player.getVelocity().horizontalLength();telemetry.history=history.snapshot().stream().map(p->{String id=Registries.BLOCK.getId(c.world.getBlockState(p).getBlock()).toString();return new Telemetry.Target(id,new Telemetry.Vec(p.getX(),p.getY(),p.getZ()),c.player.getPos().distanceTo(Vec3d.ofCenter(p)));}).toList();if(target!=null){String id=Registries.BLOCK.getId(c.world.getBlockState(target).getBlock()).toString();telemetry.target=new Telemetry.Target(id,new Telemetry.Vec(target.getX(),target.getY(),target.getZ()),c.player.getPos().distanceTo(Vec3d.ofCenter(target)));}else telemetry.target=null;}
